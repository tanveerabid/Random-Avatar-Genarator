    ...
    $.colorPick.prototype = {
        ...
        appendToStorage: function(color) {
	        if ($.fn.colorPick.defaults.allowRecent === true) {
                document.getElementById('recbackgrond').style.fill = color;
	        	var storedColors = JSON.parse(localStorage.getItem("rectanglebackground"));
				...
				if ($.inArray(color, storedColors) == -1) {
					...
					localStorage.setItem("rectanglebackground", JSON.stringify(storedColors));
	        	}
	        }
        },
        ...
        show: function(element, left, top) {
			...
			if ($.fn.colorPick.defaults.allowRecent === true) {
				...
				if (JSON.parse(localStorage.getItem("rectanglebackground")) == null || JSON.parse(localStorage.getItem("rectanglebackground")) == []) {
					$("#colorPick").append('<div class="colorPickButton colorPickDummy"></div>');
				} else {
					jQuery.each(JSON.parse(localStorage.getItem("rectanglebackground")), function (index, item) {
		        		$("#colorPick").append('<div class="colorPickButton" hexValue="' + item + '" style="background:' + item + '"></div>');
